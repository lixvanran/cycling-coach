...
def wbal_analysis(
    ...
    返回:
    {
      "cp": 280,
      "w_prime": 20000,
      "wbal_curve": [20000, 19950, 19800, ...],   # 每秒 W'bal (J)
      "min_wbal": 3200,                             # 最低值 (J)
      "min_wbal_at_s": 1820,                        # 最低值时间 (秒)
      "min_wbal_pct": 16.0,                         # 最低值百分比 (相对 W')
      "depleted": False,                            # 是否耗尽 (W'bal → 0)
      "depletion_at_s": None,                       # 耗尽时间 (None = 未耗尽)
      "critical_events": [                          # 临界事件列表 (W'bal < 30% W')
        {"start_s": 1800, "end_s": 1900, "min_wbal": 4500, "duration_s": 100}
      ],
      "match_potential": 0.65,                      # 比赛匹配潜力 (0-1)
                                                      # = 1 - (min_wbal / w_prime)
                                                      # 越高 = W' 越被榨干, 难再加速
    }

    参考:
    - Skiba et al. (2012) — W'bal 模型
    - GoldenCheetah W'bal + Skiba Critical Power 插件
