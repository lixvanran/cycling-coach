...
import { TrendsConfigBar, useTrendsConfig } from "../components/TrendsConfigBar";
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        {overview && <OverviewView overview={overview} rpe={rpe} acwr={acwr} />}
      </div>
    </div>
  );
}

function OverviewView({ overview, rpe, acwr }: { overview: any; rpe: any; acwr: any }) {
  const { volume, zones, metrics, pmc, yoy, days, weeks } = overview;
  const [config, setConfig] = useTrendsConfig();
  ...
  return (
    <>
      ...
      <div className="flex justify-end">
        ...
        <ReportDownloadButton days={7} />
        <TrendsConfigBar config={config} onChange={setConfig} />
      </div>
      </div>

      {/* 1. 训练量趋势 (TSS 柱状图) */}
      {config.volume && volume.series.length > 0 && (
      ...
      {/* 2. 7 区分布堆叠面积图 */}
      {config.zones && zones.series.length > 0 && (
        <section className="panel">
          <div className="panel-header">
      ...
      {/* 3. 关键指标趋势 (NP / IF) */}
      {config.metrics && metrics.series.length > 0 && (
        <section className="panel">
          <div className="panel-header">
      ...
      {config.rpe && rpe && rpe.series && rpe.series.length > 0 && (
        <section className="panel">
          <div className="panel-header">
