"""V0.8.0: FTP 测试业务层 (V0.6.1)

覆盖端点:
- GET  /api/ftp/methods          get_methods
- POST /api/ftp/estimate         estimate_from_activity
- POST /api/ftp/test             record_ftp_test
- GET  /api/ftp/history          list_history
- GET  /api/ftp/recommend        recommend_next_test
- DELETE /api/ftp/test/{id}      delete_test
"""
...
from cycling_coach.core.metrics.ftp import estimate_ftp as _estimate_ftp, METHODS
...
class FTPService:
    ...
    def estimate_from_activity(self, req: EstimateRequest) -> dict:
        """从已上传的活动估算 FTP (不录入, 试用)"""
        a = self.db.get(Activity, req.activity_id)
        if not a:
            raise NotFoundError(f"活动 {req.activity_id} 不存在")
        if req.method not in METHODS:
            raise ValidationError(f"未知方法: {req.method}, 可用: {list(METHODS.keys())}")
        ...
        est = _estimate_ftp(pyd, req.method, max_hr=max_hr, lthr=lthr)
        return {
            "method": est.method,
    ...
    def record_test(self, payload: RecordFTPTest) -> dict:
        """录入一次 FTP 测试结果"""
        if payload.method not in METHODS:
            raise ValidationError(f"未知方法: {payload.method}")
        if payload.ftp_w < 50 or payload.ftp_w > 600:
            raise ValidationError("ftp_w 必须在 50-600 范围 (正常人类能力)")

        try:
            test_date = datetime.fromisoformat(payload.test_date)
        except ValueError:
            raise ValidationError(f"日期格式错误: {payload.test_date}")
    ...
    def list_history(self, days: int = 365) -> list:
        """FTP 测试历史"""
        athlete = profile_store.get_or_create_athlete(self.db)
        cutoff = datetime.utcnow() - timedelta(days=days)
        tests = (
            self.db.query(FTPTest)
            .filter(FTPTest.athlete_id == athlete.id)
            .filter(FTPTest.test_date >= cutoff)
            .order_by(desc(FTPTest.test_date))
            .all()
        )
        return [self._to_out(t) for t in tests]

    def recommend_next_test(self) -> dict:
        """推荐下次测试时间"""
        athlete = profile_store.get_or_create_athlete(self.db)
        last = (
            self.db.query(FTPTest)
    ...
    def _to_out(self, t: FTPTest) -> dict:
        now = datetime.utcnow()
