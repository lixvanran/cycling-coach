...
export function ElevationProfileChart({ samples, height = 200, showClimbMarkers = true }: Props) {
  ...
  const climbSegments = useMemo(() => {
    ...
    while (i < data.length) {
      ...
      let gain = 0;
      while (j < data.length) {
        const diff = data[j].elev - data[j - 1].elev;
        if (diff > 0.3) {
          // 上升趋势
          gain += diff;
          peak = Math.max(peak, data[j].elev);
          j++;
        } else {
          break;
        }
      ...
      if (gain >= 10 && j - i >= 30) {
        segs.push({ tMin: data[i].tMin, gain: Math.round(gain), peak: Math.round(peak) });
      }
      i = j;
    }
    return segs;
  ...
  const elevMin = Math.min(...data.map((d) => d.elev));
  const elevMax = Math.max(...data.map((d) => d.elev));
  const totalGain = data.reduce((sum, d, i) => {
    if (i === 0) return 0;
    return sum + Math.max(0, d.elev - data[i - 1].elev);
  }, 0);
  const totalLoss = data.reduce((sum, d, i) => {
    if (i === 0) return 0;
    return sum + Math.max(0, data[i - 1].elev - d.elev);
  }, 0);
