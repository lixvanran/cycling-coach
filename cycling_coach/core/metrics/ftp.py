...
def estimate_ftp_coggan_20min(activity: Activity) -> FTPEstimate:
    """Coggan 经典 20 分钟测试
    ...
    为什么 × 0.95:
    - 20min 全力不可能维持 60min, 平均到 60min 大约衰减 5%
    - 来自 Coggan 实测 + 验证
    """
    ...
    if hr and ap_20min > 0:
        # 估算 %HRmax, 假设 190 默认 max
        # 高功率 + 中等心率 = 良好有氧能力
        intensity_pct = ap_20min / (ftp_w / 0.95) if ftp_w > 0 else 0  # 估算 %FTP
        if hr < 175 and ap_20min > ftp_w * 0.95:
            hr_quality = "心率协同好 (高功率低心率)"
            confidence += 0.15
        elif hr > 185:
            hr_quality = "⚠ 心率过高 (>185 bpm), 可能未充分热身"
...
def estimate_ftp_carmichael_8min(activity: Activity) -> FTPEstimate:
...
def estimate_ftp_cp_3param(activity: Activity) -> FTPEstimate:
...
def estimate_ftp_ramp(activity: Activity) -> FTPEstimate:
...
def estimate_ftp_auto(activity: Activity) -> FTPEstimate:
    """自动选择最合适的方法
    ...
    if n >= 25 * 60:
        estimates.append(estimate_ftp_coggan_20min(activity))
        estimates.append(estimate_ftp_cp_3param(activity))
    if n >= 30 * 60:
        estimates.append(estimate_ftp_carmichael_8min(activity))

    # 总是算 ramp (作为基线)
    if n >= 5 * 60:
        estimates.append(estimate_ftp_ramp(activity))
...
def estimate_ftp(activity: Activity, method: str = "auto") -> FTPEstimate:
    """主入口: 从活动估算 FTP
